...
const page = () => {
  ...
  const { data: subscriptions, isLoading: isLoading } = useSWR(
    IsCompany ? "getcompanysubscriptions" : "getbrokersubscriptions",
  ...
  return (
    <>
      <DashboardLayout panelType="admin">
        ...
        <div className="mt-10">
          <h1 className="font-bold text-[16px]">subscription</h1>
          <div className="mt-5 flex gap-3">
            <Button
              variant={IsCompany ? "contained" : "text"}
              color="primary"
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 ">
              <thead className="text-xs text-[#A0A0A0] uppercase bg-gray-50 ">
                <tr className="bg-white border-b  border-[#A0A0A0]">
                  ...
                  <th scope="col" className="px-6 py-3">
                    {IsCompany ? "Company Name" : "Broker Name"}
                  </th>
                  <th scope="col" className="px-6 py-3">
                    Expration Date
                  </th>
              ...
              <tbody>
                {subscriptions.map((item: any, i: number) => (
  ...
  const handleMakeChat = (e: any) => {
    ...
    HttpClient.getInstance()
      ...
      .then((response: any) => {
        ...
        router.push(`/dashboard/admin/messages?selected=${chatId}`);
      })
      .catch(() => {
  ...
  return (
    <>
      <tr className="bg-white border-b-2  border-[#A0A0A0]">
        <td className="px-6 py-4">{data["Purchase Date"]}</td>
        <td className="px-6 py-4">
          {data["Company Name"] || data["Broker Name"]}
        </td>
        <td className="px-6 py-4">{data["Expiration Date"]}</td>
        <td className="px-6 py-4">{data["Package Validity"]}</td>
        <td className="px-6 py-4">{data["Package Type"]}</td>
